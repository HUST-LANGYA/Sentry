...
//           waitingTick = 0,movingTick = 0;//}
    ...
    else if(LimitSw_State == Limit_Left_Active || LimitSw_State == Limit_Right_Active )  //行程开关触发
    {
			  movingDir = (LimitSw_State == Limit_Left_Active)? (MovingDir_R) : (MovingDir_L);
        waitingTick=0,movingTick = 0;
    }
    else if(Ftir_State == Ftir_Right_Active || Ftir_State == Ftir_Left_Active ) //光电开关触发,负责开始加速的过程
    {
...
//		 }
    ...
    if(waitingTick>=0 && waitingTick<50 && movingTick == 0) 
        ...
        if(waitingTick == 0 && movingTick > 10)
            motor_chassis.pid.SetPoint = (movingDir)?(-3000):(3000);  //从1速开始再加一次速的2速状态 
//        if(waitingTick == 0 && movingTick > 20)
//            motor_chassis.pid.SetPoint = (movingDir)?(-2500):(2500);  //从1速开始再加一次速的2速状态
//        if(waitingTick == 0 && movingTick > 30)
//            motor_chassis.pid.SetPoint = (movingDir)?(-3200):(3200);  //从2速开始再加一次速的3速状态  
...
inline static void Chassis_SLEEP_PID_Cal(void)
{
	 Shoot_Enable_flag = (Shoot_Enable_cnt>=1000);   
	   ...
	   float fsend;
    BodanMotor.pid_speed.SetPoint = PID_Calc(&BodanMotor.pid_pos, BodanMotor.Angle_Inc); //用位置环算速度环设定值
    fsend = PID_Calc(&BodanMotor.pid_speed, BodanMotor.RealSpeed);                       //用速度环算电流值输出
