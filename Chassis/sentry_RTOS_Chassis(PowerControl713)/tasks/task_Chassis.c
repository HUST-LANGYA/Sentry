...
//		if((rand() % 2))
//    {
//        if((last_encoder_abs < 16000)
//          && (cur_encoder_abs >= 16000)
...
static float Excess_Power=160;  //设置的超限功率
//float Chassis_Iset;
...
{ 
    ...
    //PowerLimit.SetPoint=Excess_Power*(JudgeReceive.remainEnergy-EnergyDanger)/(200-EnergyDanger);			
	   //if(movingTick>100||mid_movingTick_RtoL>150||mid_movingTick_LtoR>150)  //测试一下匀速段的功率情况
		if(motor_chassis.real_speed>5200||motor_chassis.real_speed<-5200) //这个主要是为了观察匀速段的功率使用情况,但是匀速段
		...
		//while(PowerNow>PowerLimit_1.SetPoint&&Power_De_Time<8)  //利用功率环实现功率衰减
		  while(PowerNow>PowerLimit.SetPoint&&Power_Time<8)
	{       
		     Chassis_Iset=PowerLimit_k*Chassis_Iset;
			   Chassis_Iset=LIMIT_MAX_MIN(Chassis_Iset,ChassisCurrentLimit,-ChassisCurrentLimit);
		     INA_READ_Power();
		...
		if(PowerNow>PowerLimit.SetPoint)
		{
		   Chassis_Iset=Chassis_Iset*(PowerLimit.SetPoint/PowerNow);
			 Chassis_Iset=LIMIT_MAX_MIN(Chassis_Iset,ChassisCurrentLimit,-ChassisCurrentLimit);
		}
		
		  Chassis_V=PowerNow/Chassis_Iset;
//    //功率控制相关 计算当前的缩放系数
...
{ 
	...
	PowerLimit.P= 1.0f;//0.001f;
	PowerLimit.I=10.0f; //0.0005f;
	PowerLimit.D= 0.001f;
