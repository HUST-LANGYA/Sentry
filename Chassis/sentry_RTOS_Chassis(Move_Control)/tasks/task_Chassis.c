...
uint16_t wait_time = 1; //停止下来继续观察的时间,和血量有关
...
void task_Chassis(void* parameter)
{
    ...
    while (1)
    {
        ...
        if(ABS(cur_encoder_abs-11000) < 1.5f) cross_flag = 1;  
...
//            TIM_SetCounter(TIM3,781),encoder.angle_abs = 781;   //左端光电开关触发
				...
				if(armor_state == 0x30 && motor_chassis.pid.SetPoint == 0) aim_flag = 1;
				if((last_armor_state == 0xff) && (armor_state == 0x30))  //只有从未识别状态进来的时候才会重新开始计数
					wait_time = (cur_remainHP > 300)? (3000):(1000);
...
//           waitingTick = 0,movingTick = 0;//}
    ...
    //中间段随机变向 ///嘿嘿嘿(●ˇ?ˇ●)
		...
		if(cur_remainHP > 300) //当剩余血量大于300时正常运动
	{
    if(!(rand() % 2))
    {
        ...
        if(mid_movingTick_RtoL>600) mid_reverse_flag_RtoL = 0,mid_movingTick_RtoL = 0;//中间段变向标志的超时复位
		 ...
		 // 变向的实现
			 if(rand() % 2)
    {
        if((last_encoder_abs >= 12000) //这里设置成和14200相近主要是为了能够在躲避打击的时候短距离内实现多个变向
     ...
     else if(cur_remainHP < 300 && cross_flag) //在满足血量和移动到接近到中间轨道的条件后开启多变向模式
  {
		 			  
		    if((last_encoder_abs < 13000)
          && (cur_encoder_abs >= 13000)
          && movingDir == MovingDir_R
          && !mid_reverse_flag_RtoL)//且在向右运动，没有经过右边的变向点
        {   
            mid_movingTick_RtoL=0;  //清空中间段计数器
    ...
    //瞄准的时候停下来
    if(armor_state == 0x30 && (wait_time != 0))  
		{			
		  motor_chassis.pid.SetPoint = 0; //0x30代表瞄准到了
			if(wait_time) wait_time--;
			
		}
//    Chassis_SLEEP_PID_Cal();
    if(is_Under_Attack) Chassis_Patrol_PID_Cal(); // is_Under_Attack现在是人为给定的
...
//           waitingTick = 0,movingTick = 0;//}
    ...
    //中间地带直接由遥控器来设定速度的目标值
		//motor_chassis.pid.SetPoint=kChassis*(RC_Ctl.rc.ch0-1024);//ch0控制速度值 
		 ...
		 if(PhotoEle_State == PhotoEle_Both_Active ) 
		 {
		    motor_chassis.pid.SetPoint =  0;
		 }
