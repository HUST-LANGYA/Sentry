...
uint32_t  check_Gap_Time=1200;
//功率参数
float maxPower = 20.0f;  //先改成20W
float PowerNow = 0.0f;
...
//运动状态变量
MovingDir_t movingDir,Last_movingDir;//当前行进的方向  1为向右 0为向左
...
//模糊检测的时停止相关
uint8_t aim_flag=0;
uint8_t Hide_Attack_flag;
uint8_t Left_Half_flag;
volatile uint8_t Chassis_Last_Mode;
uint8_t Chassis_Update_Flag;
uint8_t movingDir_Change_Flag;
...
void task_Chassis(void* parameter)
{
    ...
    encoder.angle_abs = 0;//假装上电时它在轨道的靠右边
    ...
    while (1)
    {
        ...
        //底盘状态更新处理			
			  ...
			  Chassis_Update_Flag = (Chassis_Last_Mode != Sentry_State.Chassis_Mode);
			  Chassis_Last_Mode = Sentry_State.Chassis_Mode;
        ...
        //check_Gap_Time = (Hide_Attack_flag)? (900) : (1300);
				
        
        //位置融合更新（用光电开关和行程开关的触发来矫正编码器的计数值）
        if(LimitSw_State == Limit_Right_Active)  
            TIM_SetCounter(TIM3,0),encoder.angle_abs = 0;  //最右端
        if(LimitSw_State == Limit_Left_Active)
            TIM_SetCounter(TIM3,22469),encoder.angle_abs = 22469;  //最左端
...
//    static uint8_t mid_reverse_flag_LtoR;//标志在中间段是变向前还是变向后
    ...
    {    
        railThreshold_L = 21000;//可以考虑尽量给大一些,左边的行程开关的灵敏度较差
        railThreshold_R = 1000;
        srand(100);  //这个随机数种子初始化和底盘的运动变向有关
    ...
    else if(encoder.angle_abs >= railThreshold_R && encoder.angle_abs <= railThreshold_L)  railSeg_Flag = RailSeg_M;
...
//			 {
//				 //根据当前编码器的值来决定向什么方向运动
//				 movingDir = (cur_encoder_abs < 11235)? (MovingDir_L):(MovingDir_R);//11235是轨道上编码器的一半值,对于不同的轨道还需要加以测试具体的数值
//				 movingTick=0;
//				 check_Gap_Time=1000;  
//			 }	
//			
...
//		{
    ...
    if(LimitSw_State == Limit_Left_Active || LimitSw_State == Limit_Right_Active )  //行程开关触发
    {
		...
		else if(LimitSw_State == Limit_Both_IDLE)
		{
		   waitingTick = 0;
			 movingTick++;
			
//			 if(movingTick>=check_Gap_Time)
//			 {
//				 Last_movingDir = movingDir;
//				 
//				 movingDir = (movingDir_Change_Flag%2)? (movingDir) : (!movingDir);
//				 if(movingDir != Last_movingDir)
//				 {	
//					 //如果方面改变,则速度先减小到0再变向
//			     movingTick=0;
//					 check_Gap_Time = 1100;
//				 }
//				 else //方向相同则间隔的时间变长,尽量以高速移动一段时间
//				 {
//				   check_Gap_Time = 1400;
//				 }
//			 }
    ...
    //根据各种计数变量的值，改变运动状态
    if(waitingTick<30 && movingTick == 0) 
        motor_chassis.pid.SetPoint = 0;//暂停状态0速
    else if(waitingTick>=30 && movingTick == 0)
        ...
        if(waitingTick == 0 && movingTick > 40)
        ...
        if(waitingTick == 0 && movingTick > 80)
        ...
        if(waitingTick == 0 && movingTick > 120 /*&& movingTick < 100*/)  
            motor_chassis.pid.SetPoint = (movingDir)?(-5500):(5500);  
        if(waitingTick == 0&& movingTick>150)
				{
					  motor_chassis.pid.SetPoint = (movingDir)?(-5500):(5500);					
...
//   if(movingDir == MovingDir_L && railSeg_Flag == RailSeg_L) 
//	 motor_chassis.pid.SetPoint = (movingDir)?((ABS(motor_chassis.pid.SetPoint) > 3000)? (-3000) : (motor_chassis.pid.SetPoint)):((ABS(motor_chassis.pid.SetPoint)>3000)? (3000) : (motor_chassis.pid.SetPoint)); 
//   if(movingDir == MovingDir_R && railSeg_Flag == RailSeg_R)  
//	 motor_chassis.pid.SetPoint = (movingDir)?((ABS(motor_chassis.pid.SetPoint) > 3000)? (-3000) : (motor_chassis.pid.SetPoint)):((ABS(motor_chassis.pid.SetPoint)>3000)? (3000) : (motor_chassis.pid.SetPoint)); 
		
    //瞄准的时候停下来
    //if((armor_state == 0x30) && (cur_remainHP>500))   motor_chassis.pid.SetPoint = 0;
//	}
//    Chassis_SLEEP_PID_Cal();
    if(is_Under_Attack /*&& defend_flag|| cur_escape_period < set_escape_period*/) Chassis_Patrol_PID_Cal(); // is_Under_Attack现在是人为给定的
    else Chassis_SLEEP_PID_Cal();
}
...
static float EnergyDanger=50;   //缓冲能量危险值
static float Excess_Power=40;  //设置的超限功率//改小了主要是怕限不住啊因为正常的时间太长了
//float Chassis_Iset;
...
{ 
    ...
    Chassis_Iset = LIMIT_MAX_MIN(PID_Calc( &motor_chassis.pid, motor_chassis.real_speed), ChassisCurrentLimit, -ChassisCurrentLimit);
	  ...
	  //else if(movingTick<=100||mid_movingTick_RtoL<=150||mid_movingTick_LtoR<=150)  //这些需要根据缓冲能量来进一步确定看加速度确定为多少合适
		else if(motor_chassis.real_speed<5500&&motor_chassis.real_speed>-5500)
		{
...
{ 
  ...
  motor_chassis.pid.OutMax = 10000.0f;//10000.0f;//5000.0f;
