...
uint16_t is_Under_Attack = 1; //如果受击，则将该变量置于某个单稳态的值
	...
	for (PackPoint = 0; PackPoint < JudgeBufBiggestSize; PackPoint++)				  //先处理前半段数据(在上一周期已接收完成)
	{
		if (SaveBuffer[PackPoint] == 0xA5)
		{
			if ((Verify_CRC8_Check_Sum(&SaveBuffer[PackPoint], 5) == 1)) //frame_header 五位数据校验
			{
				...
				DataLen = SaveBuffer[PackPoint + 2] & 0xff; //高字节数据后发
				DataLen = (DataLen << 8) | SaveBuffer[PackPoint + 1]; //低字节数据先发
				//机器人状态数据
				num_judge_pack++;
				...
				if ((cmd_id == 0x0003) && (Verify_CRC16_Check_Sum(&SaveBuffer[PackPoint], DataLen + 9)))
				{
				   JudgeReceive.red_outpost_hp = SaveBuffer[PackPoint + 7 + 12] << 8 | SaveBuffer[PackPoint + 7 + 13];
					 ...
					 defend_flag = (JudgeReceive.red_outpost_hp ==0) | (JudgeReceive.blue_outpost_hp == 0); //前哨站被打爆的标志
				}
				if ((cmd_id == 0x0201) && (Verify_CRC16_Check_Sum(&SaveBuffer[PackPoint], DataLen + 9)))
				{
					...
					JudgeReceive.HeatCool17_2 = (SaveBuffer[PackPoint + 7 + 13] << 8) | SaveBuffer[PackPoint + 7 + 12];
					JudgeReceive.HeatMax17_2 = (SaveBuffer[PackPoint + 7 + 15] << 8) | SaveBuffer[PackPoint + 7 + 14];
					JudgeReceive.BulletSpeedMax17_2 = (SaveBuffer[PackPoint + 7 + 17] << 8) | SaveBuffer[PackPoint + 7 + 16];
				...
				if ((cmd_id == 0x0202) && (Verify_CRC16_Check_Sum(&SaveBuffer[PackPoint], DataLen + 9)))
				{
					...
					JudgeReceive.realChassisOutV = JudgeReceive.realChassisOutV / 1000.0f; //为了统一单位?
				...
				if((cmd_id == 0x208) && (Verify_CRC16_Check_Sum(&SaveBuffer[PackPoint], DataLen + 9)))
				{
					//获取当前子弹的数目用于决策
				   JudgeReceive.bullet_remain_num = (SaveBuffer[PackPoint + 7 + 0] << 8) | (SaveBuffer[PackPoint + 7 + 1]);
				
				}
				if ((cmd_id == 0x0301) && (Verify_CRC16_Check_Sum(&SaveBuffer[PackPoint], DataLen + 9)))
				{ //飞机给一个信号，让哨兵伪受击，开始底盘运动
					sender_ID = SaveBuffer[PackPoint + 7 + 3] << 4 | SaveBuffer[PackPoint + 7 + 2]; //这里只移动了四位是因为通信的最大值是0X2FF
					...
					if (sender_ID == 6 && receiver_ID == 7) //红方
					{
					...
					else if (sender_ID == 106 && receiver_ID == 107) //蓝方
					{
						is_Under_Attack = 10000;
						cur_escape_period = 0;
					}
