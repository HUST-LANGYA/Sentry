...
void ADC_Config(void)
{
    GPIO_InitTypeDef gpio;
	  ...
	  gpio.GPIO_Pin=GPIO_Pin_1/*|GPIO_Pin_2*/; //注意这里和光电传感器的引脚可能会冲突,调不出来的时候可以看看
    ...
    gpio.GPIO_Speed=GPIO_Speed_50MHz;
	  ...
	  ADC_DeInit(ADC1);
		...
		adc1.ADC_NbrOfChannel=1; //扫描4个通道
		ADC_Init(ADC1,&adc1);
		
		ADC_TempSensorVrefintCmd(ENABLE);//温度传感器使用
	RCC_ADCCLKConfig(RCC_PCLK2_Div6);
	ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 1, ADC_SampleTime_239Cycles5);	//u_in
     ...
     if(DMA_GetFlagStatus(DMA1_IT_TC1)!=RESET)
		 {
			...
			//				Actual_Value = Actual_Value/2*0.0048828125; //(5/1024 = 0.00488)
				Actual_Value = (((Sum_Value/2)*5)/4096);//还可以
				Sum_Value = 0;
				distance  = 12*pow(Actual_Value,-1.058);//测不对
		    DMA_Cmd(DMA1_Channel1,ENABLE);
		 }
}
