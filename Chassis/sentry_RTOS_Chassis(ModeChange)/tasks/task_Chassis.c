...
uint16_t Patrol_Flag=0;
uint16_t Hide_Time=10000;
uint16_t mid_reverse_init=0;
//光电开关错误检测标志
...
uint32_t mid_movingTick; //记录中间段的时间
static void Chassis_Patrol_Act(void)
{
//    //轨道分段变量
//    static uint16_t  railThreshold_R;  
...
//           waitingTick = 0,movingTick = 0;//}
    ...
    if(encoder.angle_abs < railThreshold_L)  railSeg_Flag = RailSeg_L;
		...
		//错误状态判断
		 Photo_Error_Flag=((PhotoEle_State==PhotoEle_Right_Active)&&(railSeg_Flag==RailSeg_M));
			...
			if(!(rand() % 2))
    {
        ...
        if(mid_movingTick_RtoL>600) mid_reverse_flag_RtoL = 0,mid_movingTick_RtoL = 0;//中间段变向标志的超时复位
		 ...
		 // 变向的实现
			 if(rand() % 2)
    {
        if((last_encoder_abs >= 9000) //这里设置成和14200相近主要是为了能够在躲避打击的时候短距离内实现多个变向
...
//    if(movingDir == MovingDir_R && railSeg_Flag == RailSeg_R)  motor_chassis.pid.SetPoint = (movingDir)?(-3000):(3000); 
   ...
   else if(is_Under_Attack == 1) //受到打击后
  {
		 if(((cur_encoder_abs-15200)<1)&&(mid_reverse_init != 1)) // 在一定的范围内即可
		 {
		 ...
		 if(mid_reverse_flag_RtoL)
		 {
		     mid_movingTick++;      //在中间段某一端的时间统计
         mid_movingTick_RtoL++; //作用加速阶段的时间统计
         if(mid_movingTick_RtoL >10 )   { motor_chassis.pid.SetPoint = 0;movingDir = MovingDir_L; }//先变向，再加速
         //if(mid_movingTick_RtoL > 20)  motor_chassis.pid.SetPoint = (movingDir)?(-1800):(1800);
         //if(mid_movingTick_RtoL > 30)  motor_chassis.pid.SetPoint = (movingDir)?(-2400):(2400);
         if(mid_movingTick_RtoL > 40)  motor_chassis.pid.SetPoint = (movingDir)?(-3000):(3000);
         ...
         if(mid_movingTick_RtoL > 70)  motor_chassis.pid.SetPoint = (movingDir)?(-4800):(4800);
         if(mid_movingTick_RtoL > 80)  motor_chassis.pid.SetPoint = (movingDir)?(-5200):(5200);       
				 if(mid_movingTick_RtoL > 90)  //加速阶段完成之后实现反向
         { 
					motor_chassis.pid.SetPoint = (movingDir)?(-5800):(5800);
				  mid_movingTick_RtoL=0;
					 movingDir=(MovingDir_R)? (MovingDir_L):(MovingDir_R); 
				 } 
				 if(mid_movingTick>4000) 
				 { 
				   mid_reverse_flag_RtoL = 0;
					 mid_movingTick = 0;
					 movingDir = MovingDir_L;
					 motor_chassis.pid.SetPoint = (movingDir)?(-4800):(4800);
				 }
		 }

	   
		 
		 if(((cur_encoder_abs-12000)<1) && mid_reverse_init != 2) //进入中间段左边
		 {
		    mid_movingTick_LtoR = 0;
		    mid_reverse_flag_LtoR = 1;
			  motor_chassis.pid.SetPoint = (movingDir)?(-3000):(3000); //先减一段速 
			  mid_reverse_init = 2;
		 }
		
		 if(mid_movingTick_LtoR)
		 {
		    mid_movingTick++; 
		    mid_movingTick_LtoR++;
         ...
         if(mid_movingTick_LtoR > 70)  motor_chassis.pid.SetPoint = (movingDir)?(-4800):(4800);
         if(mid_movingTick_LtoR > 80)  motor_chassis.pid.SetPoint = (movingDir)?(-5200):(5200);     
         if(mid_movingTick_LtoR > 90) 
         { 
					 motor_chassis.pid.SetPoint = (movingDir)?(-5800):(5800);
				   mid_movingTick_LtoR=0;
				   movingDir= (MovingDir_L)? (MovingDir_R):(MovingDir_L); 
				 }
		     if(mid_movingTick>4000)
				 {
...
//    Chassis_SLEEP_PID_Cal();
    if(Patrol_Flag /*|| cur_escape_period < set_escape_period*/) Chassis_Patrol_PID_Cal(); // is_Under_Attack现在是人为给定的
...
//			 Balance_Power=30.0f;   //0.9*30.0
		...
		//while(PowerNow>PowerLimit_1.SetPoint&&Power_De_Time<8)  //利用功率环实现功率衰减
...
static void PID_Chassis_Init(void)
{ 
  motor_chassis.pid.P = 40.0f;//52.0f;//50.0f;//0.8f;
