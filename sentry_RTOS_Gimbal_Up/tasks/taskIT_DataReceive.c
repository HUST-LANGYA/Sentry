...
void PCReceive(uint8_t /*unsigned char*/ PCReceivebuffer[])
{
    ...
    distance = (short)PCReceivebuffer[8] /*>>1*/; //7~1位给到distance
...
//    if (Buf[0] == '!' && Buf[7] == '#')
//    {
//        if (Verify_CRC8_Check_Sum(&Buf[1], 10))
//        {
//            //block_disconnect.PC_Last_Cnt = GetSysCnt();
////            if (Sentry_State.Gimbal_Up_Mode == Gimbal_Up_PC
////                || Sentry_State.Gimbal_Up_Mode == Gimbal_Up_DEBUG)
//            {
//                if (Buf[1] == 0x05)
//                {
