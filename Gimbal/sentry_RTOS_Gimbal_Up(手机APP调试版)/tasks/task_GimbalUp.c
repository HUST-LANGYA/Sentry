...
static void Gimbal_DEBUG_Act(void)
{
    ...
    Test_Yaw_Pos = MotoYaw.PidPos.SetPoint;                             //+ YAW_GYRO_OFFSET;       //RCYaw_Scal * (RC_Ctl.rc.ch2 - 1024);
    Test_Pitch_Pos = MotoPitch.PidPos.SetPoint;       /*+ PITCH_GYRO_OFFSET*/; //RCPitch_Scal * (RC_Ctl.rc.ch3 - 1024) + PITCH_GYRO_OFFSET;	//   MotoPitch.PidSpeed.SetPoint = Test_Pitch_Veloc/*+ PITCH_GYRO_OFFSET*/;   //RCPitch_Scal * (RC_Ctl.rc.ch3 - 1024) + PITCH_GYRO_OFFSET;                                                          //   MotoYaw.PidSpeed.SetPoint = Test_Yaw_Veloc;
...
/**
  ...
  * @retval None
  */
float patrol_step_pitch = 0.035f/*0.035f */, patrol_step_yaw =0.010f;// 0.005f;
...
{
    PITCH_MAX_ANGLE =+30.0f; //尽量不要打太远，会被对面的高地挡住
    PITCH_ZERO_POS = +0;
    ...
    PATROL_PITCH_MAX_ANGLE =+30.0f; //-3.0f;
    PATROL_PITCH_ZERO_POS = +0;
    PATROL_PITCH_MIN_ANGLE =-30.0f; //-30.0f;
...
static void PID_Gimbal_Init(void)
{
    MotoPitch.PidPos.P = -450.0f;//-500.0f;   //-450.0f
    MotoPitch.PidPos.I = -1.5f;//-1.5f;     //-0.7f
    MotoPitch.PidPos.D = 0.0f;
    MotoPitch.PidPos.IMax = 1000.0f;

    MotoPitch.PidSpeed.P = -2500.0f;//-6500.0f;//-8000.0f; //-2500.0f
    MotoPitch.PidSpeed.I = -16.0f;//-5.0f;               //-16.0f
...
//    MotoPitch.PidPosV.IMax = 1200.0f;
    ...
    MotoPitch.PidPosV.P = -250.0f;//-300.0f;//-280.0f;//-100.0f;//-200.0f; //-300.0f;
    ...
    MotoPitch.PidSpeedV.P = -4000.0f;//-5000.0f;//-6000.0f; //-5000.0f;
    ...
    //////////////////////////////////////////////////////////////
    MotoYaw.PidPos.P = 0.35f;//0.65f; //0.5f
    MotoYaw.PidPos.I = 0.0f;//0.0008f; //0.0008f
    MotoYaw.PidPos.D = 0.0f;//3.8f;
    MotoYaw.PidPos.I_Limit = 0.5;
    MotoYaw.PidPos.IMax = 5000.0f;

    MotoYaw.PidSpeed.P = 7000.0f;//8000.0f; //7000.0f;
    MotoYaw.PidSpeed.I = 10.0f;//2.0f;    //10.0f;
    MotoYaw.PidSpeed.D = 0.0f;    
    MotoYaw.PidSpeed.I_Limit = 1;
    MotoYaw.PidSpeed.IMax = 1000.0f;

    MotoYaw.PidPosV.P = 0.36f;//0.01f;//0.5f;//0.01f;//0.05f;
    MotoYaw.PidPosV.I = 0.0015f;//0.0008f;//0.0f;
    MotoYaw.PidPosV.D = 0.0f;//2.0f;//3.8f;//2.0f;
    MotoYaw.PidPosV.I_Limit = 0.0f;
    MotoYaw.PidPosV.IMax =600; //5000.0f;//0.0f;

    MotoYaw.PidSpeedV.P = 9000.0f; //8000.0f
    MotoYaw.PidSpeedV.I = 10.0f;//2.0f;
