    ...
    if (DMA_GetITStatus(DMA1_Stream5, DMA_IT_TCIF5))
    {
        ...
        for (PackPoint = 0; PackPoint < PC_RECVBUF_SIZE; PackPoint++) //防止错位，不一定数组元素的第一个就为
        {
            if (temptemp[PackPoint] == '!')
            {
				...
				//Com_Offet.Uart_Offet.Uart_PC_Rec_CurTime++;
				block_disconnet.PC_Rec_Last_Cnt = xTaskGetTickCount();;
...
void DMA1_Stream6_IRQHandler(void)
{
    if (DMA_GetFlagStatus(DMA1_Stream6, DMA_IT_TCIF6) == SET)
    {
        ...
        DMA_ClearITPendingBit(DMA1_Stream6, DMA_IT_TCIF6);
			  block_disconnet.PC_Send_Last_Cnt = xTaskGetTickCount();
			  //Com_Offet.Uart_Offet.Uart_PC_Send_CurTime++;
    }
}
