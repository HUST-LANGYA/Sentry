...
static void aiming(void)
{
    ...
    if (armor_state == ARMOR_AIMED &&
        ...
        CV_Shoot_ABLE = 1;
			  ...
			  //last_armor_state = ARMOR_NO_AIM; //清除瞄准的标志位
		}
    else 
		{
		  CV_Shoot_ABLE = 0;
		  //last_armor_state = ARMOR_NO_AIM; //对其清零
		}
...
static void Shoot_RC_Act(void)
{
    ...
    FrictionWheel_SetSpeed(FrictionWheel_L_Speed_High, FrictionWheel_R_Speed_High);
...
static void PID_Shoot_Init(void)
{
    ...
    FrictionMotor[0].pid_speed.IMax = 100;
    ...
    FrictionMotor[1].pid_speed.IMax = 100;//0.0f;
