...
uint8_t block_change_flag = 0;
//******************内部函数声明***********************************************//
void PID_Shoot_Init(void);  //初始化bodan电机的PID参数
...
void task_ShootDn(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (block_bullet_tick >= 100) //此处改为0.5s,主要是测试时最好这样
...
float testInc = 36890.0f;//29510.0f;          //拨一颗弹丸需给到pos的增量值 (这个值比较难调到刚好拨走一颗 8192*36/10 = 29491)
uint32_t delayTick_oneShot = 50; //走掉一颗弹丸间隔的时间(ms)（一般调试的时候设在1000ms~2000ms之间可能比较合适）
...
//    }
    ...
    //BodanMotor.pid_pos.SetPoint = bodanLastPos + testInc;                                   //位置值设定为当前值增加一格弹丸的角度
	  BodanMotor.pid_pos.SetPoint=Bodan_Debug;
...
//int16_t FrictionWheel_Debug_Right=8500;
//int16_t FrictionWheel_Debug_Left=8500;
int16_t FrictionMotor_Debug=8500; //还是设成一样吧，就离谱
int16_t RC_test_fric_speed0, RC_test_fric_speed1;
static void Shoot_RC_Act(void)
{
	   RC_test_fric_speed0=FrictionMotor_Debug,RC_test_fric_speed1=FrictionMotor_Debug;
        ...
        if (shootDelayTick >= delayTick_oneShot)
        {
           ...
           BodanMotor.pid_pos.SetPoint = bodanLastPos + testInc; //位置值设定为当前值增加一格弹丸的角度
        else
				{
					 ...
					 {
						BodanMotor.pid_pos.SetPoint = bodanLastPos - (testInc/5.0f);
						block_change_flag = 1;
					 }
					 else
					 {
					   BodanMotor.pid_pos.SetPoint = bodanLastPos + 49152.0f; //位置值设定为当前值增加一格弹丸的角度
						 block_change_flag = 0;
					 }
...
void PID_Shoot_Init(void)
{
    ...
    BodanMotor.pid_speed.I = 0.0f; //3.0f
    BodanMotor.pid_speed.D = 0.0f; 
    BodanMotor.pid_speed.IMax = 0; //250
    BodanMotor.pid_pos.P = 0.15f;
    BodanMotor.pid_pos.I = 0.1f;//0.0f;
    BodanMotor.pid_pos.D = 0.0f;//22.0f;
    BodanMotor.pid_pos.IMax = 100;//0.0f;//100
    ...
    FrictionMotor[0].pid_speed.P = 120.0f;//20.0f;//36.0f;//26.0f; //120.0 //150
    FrictionMotor[0].pid_speed.I = 0.5f;//0.1f;//0.0f; //0.5f;    //0.5f
    FrictionMotor[0].pid_speed.D = 30.0f;//0.0f;//8.0f;//0.0f;//0.001f;   //30.0f
    FrictionMotor[0].pid_speed.IMax = 100.0f;//0;//80.0f;
    ...
    FrictionMotor[0].zerocheck.LastValue = BodanMotor.Angle_ABS;

    FrictionMotor[1].pid_speed.P = 100.0f;//28.0f;//20.0f;//36.0f;//16.0f;//24.0f;//30.0f; //100.0f
    FrictionMotor[1].pid_speed.I = 1.2f;//1.0f;//0.1f;//0.0f;//0.8f; //1.2f;
    FrictionMotor[1].pid_speed.D = 0.0f;//8.0f;//0.0f;//0.001f;
    FrictionMotor[1].pid_speed.IMax = 100.0f;//0; //100.0f; //120
    ...
    FrictionMotor[1].zerocheck.LastValue = BodanMotor.Angle_ABS;
}
