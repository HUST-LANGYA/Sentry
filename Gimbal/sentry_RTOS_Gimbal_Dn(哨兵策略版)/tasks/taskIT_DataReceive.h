...
#define MotoPitch_ID_Dn 0x205
#define MotoYaw_ID_Dn 0x206
...
#define BodanMotor_Dn_ID 0x204	//下云台拨弹电机的报头ID（上拨弹的是0x203）
...
typedef struct
{
  float Up_pitch;
	float Up_yaw;
	float Down_Trans_pitch; //pitch角度转化后的输出值
	float Up_distance;
	float Down_Trans_yaw;   //yaw角度转化后的数出值
	uint8_t Valid_flag;
