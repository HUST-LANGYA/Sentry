...
{
    PITCH_MAX_ANGLE =30; //尽量不要打太远，会被对面的高地挡住
    PITCH_ZERO_POS = +0;
    PITCH_MIN_ANGLE =-32; //太大好像会出现问题
...
static void PID_Gimbal_Init(void)
{
    MotoPitch.PidPos.P = -450.0f;
    MotoPitch.PidPos.I = -0.4f;//-1.5f;
    MotoPitch.PidPos.D = 0.0f;
    MotoPitch.PidPos.IMax = 1000.0f;

    MotoPitch.PidSpeed.P = -7200.0f;//-6500.0f; //-8000.0f;
    MotoPitch.PidSpeed.I = -6.0f;
