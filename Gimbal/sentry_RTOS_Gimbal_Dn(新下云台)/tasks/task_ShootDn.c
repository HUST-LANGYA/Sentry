...
#define BodanDelay_Threshold 1000                    //存放拨弹延时的上门限
...
static void Shoot_PC_Act(void)
{
    FrictionWheel_SetSpeed(FrictionWheel_L_Speed_Med, FrictionWheel_R_Speed_Med); //*设置摩擦轮
    extern uint8_t CV_Shoot_ABLE;                                                 //判定视觉方面是否能够打子弹
    //aiming();
...
int16_t RC_test_fric_speed0 = /*5000*/FrictionWheel_L_Speed_High, RC_test_fric_speed1 = /*5000*/FrictionWheel_R_Speed_High;
float RC_Bodan;
static void Shoot_RC_Act(void)
{
...
static void PID_Shoot_Init(void)
{
    ...
    BodanMotor.pid_pos.P = 0.28f;//0.25f;//0.15f;
    BodanMotor.pid_pos.I = 0.08f;//0.0f;
    BodanMotor.pid_pos.D = 0.0f;//22.0f;
    BodanMotor.pid_pos.IMax = 700.0f;//0.0f;
    ...
    FrictionMotor[0].pid_speed.I = 0.2f;//1.2f;//0.0f;//0.6f;
    FrictionMotor[0].pid_speed.D = 0.0f; //0.0f
    FrictionMotor[0].pid_speed.IMax = 1000.0f;//1500.0f;//800.0f;//100.0f; //0.0f//1500.0f;
    ...
    FrictionMotor[1].pid_speed.I = 0.2f;//1.2f;//0.0f;//0.6f;
    FrictionMotor[1].pid_speed.D = 0.0f;
    FrictionMotor[1].pid_speed.IMax = 1000.0f;//1500.0f;//800.0f;//100.0f;//0;//1500.0f;
