...
float RCPitch_Scal = 0.00015f, RCYaw_Scal = 0.00012f; //遥控器对两轴控制的放大比率
...
static void Gimbal_RC_Act(void)
{
    ...
    if (GimbalDn_ModeUpdate_Flag)
    {
        ...
        MotoPitch.PidPos.SetPoint = Pitch_Actual;
			  ...
			  MotoYaw.PidPos_Fuzzy.SetPoint = Yaw_Actual;
			  MotoPitch.PidPos_Fuzzy.SetPoint = Pitch_Actual;
    }
    else
    { //如果模式没有发生变化，则正常执行遥控器赋值的部分
        ...
        MotoPitch.PidPos.SetPoint += RCPitch_Scal * (RC_Ctl.rc.ch3 - 1024); // + PITCH_GYRO_OFFSET;
			//模糊PID
			  MotoYaw.PidPos_Fuzzy.SetPoint -= RCYaw_Scal * (RC_Ctl.rc.ch2 - 1024);
			  MotoYaw.PidPos_Fuzzy.SetPoint = LIMIT_MAX_MIN(MotoYaw.PidPos_Fuzzy.SetPoint, YAW_MAX_ANGLE, YAW_MIN_ANGLE);
			  MotoPitch.PidPos_Fuzzy.SetPoint += RCPitch_Scal * (RC_Ctl.rc.ch3 - 1024);
    }
...
//            MotoYaw.PidPosV.SetPoint = aim_yaw;
        ...
        {    //若相机未捕捉到装甲，则进入巡逻模式，云台姿态以一个特定的步长来步进
					  extern float aim_Down_Pitch;//下云台发来的Pitch协同值
            extern float aim_Down_Yaw;//下云台发来的Yaw协同值
					
					  if(aim_Valid_Flag)
						{
						    MotoPitch.PidPosV_Fuzzy.SetPoint = LIMIT_MAX_MIN(aim_Down_Pitch,PATROL_PITCH_MAX_ANGLE,PATROL_PITCH_MIN_ANGLE);
							  MotoYaw.PidPosV_Fuzzy.SetPoint = LIMIT_MAX_MIN(aim_Down_Pitch,PATROL_YAW_MAX_ANGLE,PATROL_YAW_MIN_ANGLE);
            ...
            MotoPitch.PidPosV_Fuzzy.SetPoint = LIMIT_MAX_MIN(patrol_set_pitch, PATROL_PITCH_MAX_ANGLE, PATROL_PITCH_MIN_ANGLE);
            MotoYaw.PidPosV_Fuzzy.SetPoint = LIMIT_MAX_MIN(patrol_set_yaw, PATROL_YAW_MAX_ANGLE,PATROL_YAW_MIN_ANGLE);
...
//    MotoPitch.I_Set = LIMIT_MAX_MIN(fsend, gyroLimitPitch, -gyroLimitPitch);
    ...
    //模糊PID
	  MotoPitch.PidSpeed_Fuzzy.SetPoint = FuzzyPID_Calc(&MotoPitch.PidPos_Fuzzy,Pitch_Actual) / 1000.0f;
	  fsend_fuzzy = FuzzyPID_Calc(&MotoPitch.PidSpeed_Fuzzy,Gyro_White.GY);
...
//    MotoYaw.I_Set = LIMIT_MAX_MIN(fsend, gyroLimitYaw, -gyroLimitYaw);
	  ...
	  //Yaw轴模糊PID解算
	  MotoYaw.PidSpeed_Fuzzy.SetPoint = FuzzyPID_Calc(&MotoYaw.PidPos_Fuzzy,Yaw_Actual) / 1000.0f;
	  fsend_fuzzy = FuzzyPID_Calc(&MotoYaw.PidSpeed_Fuzzy,Gyro_White.GZ);
...
static void Gimbal_GYRO_Cal(void)
{
    if (init_comb_flag)
    {
        ...
        moto_yaw_init = 1383.0f;//1383.0f;//YawAngleOutPut();//3800.0f;
		...
		{
		   //moto_pitch_init = 
			 moto_yaw_init = 5479.0f;
		}
		else
		{
		  //moto_pitch_init = 
			moto_yaw_init = 1383.0f;
		}
...
static void Gimbal_Limit(void)
{
    ...
    PATROL_YAW_MAX_ANGLE = /*YAW_GYRO_OFFSET +*/ 60; //Yaw_Actual + 30;
    PATROL_YAW_ZERO_POS = /*YAW_GYRO_OFFSET + */ 0;  //Yaw_Actual + 0;
    PATROL_YAW_MIN_ANGLE = /*YAW_GYRO_OFFSET */ -60; //Yaw_Actual - 30;
}
...
//    MotoPitch.PidPosV.IMax = 800.0f;
    ...
    MotoPitch.PidSpeedV.IMax = 500.0f;
		...
		//FUZZY_PID
		MotoPitch.PidPos_Fuzzy.Kp = 250.0f;;
		MotoPitch.PidPos_Fuzzy.Ki = 0.5f;
		MotoPitch.PidPos_Fuzzy.Kd = 0.0f;
		MotoPitch.PidPos_Fuzzy.IMax = 800.0f;
		
		MotoPitch.PidSpeed_Fuzzy.Kp = 5000.0f;;
		MotoPitch.PidSpeed_Fuzzy.Ki = 8.5f;
		MotoPitch.PidSpeed_Fuzzy.Kd = 0.0f;
		MotoPitch.PidSpeed_Fuzzy.IMax = 500.0f;
		
		MotoPitch.PidPosV_Fuzzy.Kp = 390.0f;//350.0f;
    MotoPitch.PidPosV_Fuzzy.Ki = 1.6f;//3.8f;
		MotoPitch.PidPosV_Fuzzy.Kd = 0.0f;
		MotoPitch.PidPosV_Fuzzy.IMax = 1500.0f;
		
		MotoPitch.PidSpeedV_Fuzzy.Kp = 5000.0f;
		MotoPitch.PidSpeedV_Fuzzy.Ki = 15.0f;
		MotoPitch.PidSpeedV_Fuzzy.Kd = 0.0f;
		MotoPitch.PidSpeedV_Fuzzy.IMax = 500.0f;
...
//    MotoPitch.PidSpeedV.IMax = 0.0f;
    ...
    MotoYaw.PidPos.P = 250.0f;//480.0f;
    MotoYaw.PidPos.I = 0.45f;//0.35f;
    MotoYaw.PidPos.D = 0.0f;//925.0f;
    MotoYaw.PidPos.IMax = 120.0f;//500.0f;

    MotoYaw.PidSpeed.P = -12000.0f;//-9000.0f;
    MotoYaw.PidSpeed.I = -15.0f;//-2.0f;//-5.0f;
    MotoYaw.PidSpeed.D = 0.0f;
    MotoYaw.PidSpeed.IMax = 300.0f;

 //没有除以1000所以会和其他模式差得比较大
    MotoYaw.PidPosV.P = 0.4f;//0.4f; //-280.0f;
    MotoYaw.PidPosV.I = 0.0022f;//0.002f; //-0.35f;
    MotoYaw.PidPosV.D = 0;//925.0f;
    MotoYaw.PidPosV.IMax = 300.0f;//0;//500.0f;

    MotoYaw.PidSpeedV.P = -11000.0f;//-8000.0f;
    MotoYaw.PidSpeedV.I = -15.0f;//-5.0f;
    MotoYaw.PidSpeedV.D = 0.0f;
    MotoYaw.PidSpeedV.IMax = 300.0f;

    //FUZZY_PID
    MotoYaw.PidPos_Fuzzy.Kp = 200.0f;
		MotoYaw.PidPos_Fuzzy.Ki = 0.45f;
		MotoYaw.PidPos_Fuzzy.Kd = 0.0f;
		MotoYaw.PidPos_Fuzzy.IMax = 120.0f;
		
		MotoYaw.PidSpeed_Fuzzy.Kp = -12000.0f;
		MotoYaw.PidSpeed_Fuzzy.Ki = -15.0f;
		MotoYaw.PidSpeed_Fuzzy.Kd = 0.0f;
		MotoYaw.PidSpeed_Fuzzy.IMax = 300.0f;
		
		MotoYaw.PidPosV_Fuzzy.Kp = 0.35f;
    MotoYaw.PidPosV_Fuzzy.Ki = 0.0020f;
		MotoYaw.PidPosV_Fuzzy.Kd = 0.0f;
		MotoYaw.PidPosV_Fuzzy.IMax = 300.0f;
		
		MotoYaw.PidSpeedV_Fuzzy.Kp = -11000.0f;
		MotoYaw.PidSpeedV_Fuzzy.Ki = -15.0f;
		MotoYaw.PidSpeedV_Fuzzy.Kd = 0.0f;
		MotoYaw.PidSpeedV_Fuzzy.IMax = 300.0f;
