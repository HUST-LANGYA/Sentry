...
//		DMA_ITConfig(DMA1_Stream5,DMA_IT_TC,ENABLE);   //个人感觉，这个中断开归开，但不要在这个里面处理接收的数据会比较好
...
void USART2_IRQHandler(void)
{
    if (USART_GetITStatus(USART2, USART_IT_IDLE) != RESET)
    {
        ...
        DMA_Cmd(DMA1_Stream5, DISABLE); //先关DMA
        Judge_Receive(JudgeReceiveBuffer);  //处理从裁判系统接收的数据
        DMA_SetCurrDataCounter(DMA1_Stream5, JudgeBufBiggestSize);  //重新装填待传输的字节数（不过我感觉 循环模式下，不手动装填应该也行吧？）
        DMA_Cmd(DMA1_Stream5, ENABLE);  //重开DMA 
    }
}
