...
static void get_robot_status(uint8_t DataBuffer[], uint16_t data_length, ext_game_robot_status_t* robot_status)
{
    ...
    /*每秒冷却值*/
    /*枪口热量上限*/
    robot_status->shooter_heat0_cooling_rate = ((uint16_t)DataBuffer[7] << 8 | DataBuffer[6]);
    robot_status->shooter_heat0_cooling_limit = ((uint16_t)DataBuffer[9] << 8 | DataBuffer[8]);
    robot_status->shooter_heat1_cooling_rate = ((uint16_t)DataBuffer[11] << 8 | DataBuffer[10]);
    robot_status->shooter_heat1_cooling_limit = ((uint16_t)DataBuffer[13] << 8 | DataBuffer[12]);
    
    /*枪口上限速度*/
    robot_status->shooter_heat0_speed_limit = DataBuffer[14];
    robot_status->shooter_heat1_speed_limit = DataBuffer[15];
    
    //机器人最大底盘功率
    robot_status->max_chassis_power = DataBuffer[16];
...
static void get_power_heat_data(uint8_t DataBuffer[], uint16_t data_length, ext_power_heat_data_t* power_heat_data)
{
    ...
    power_heat_data->shooter_heat0 = ((uint16_t)DataBuffer[11] << 8 | DataBuffer[10]);
    power_heat_data->shooter_heat1 =((uint16_t)DataBuffer[13] << 8 | DataBuffer[12]);
    power_heat_data->mobile_shooter_heat2 = ((uint16_t)DataBuffer[15] << 8 | DataBuffer[14]);
}
