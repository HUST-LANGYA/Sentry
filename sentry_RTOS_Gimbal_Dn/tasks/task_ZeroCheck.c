...
void task_ZeroCheck(void)
{
    ...
    while (1)
    {
        ...
        FrictionMotor[0].Angle_Inc = ZeroCheck(&FrictionMotor[0].zerocheck, FrictionMotor[0].Angle_ABS, Position);     //拨弹电机
        FrictionMotor[1].Angle_Inc = ZeroCheck(&FrictionMotor[1].zerocheck, FrictionMotor[1].Angle_ABS, Position);     //拨弹电机
        vTaskDelay(1); 
    }
}
