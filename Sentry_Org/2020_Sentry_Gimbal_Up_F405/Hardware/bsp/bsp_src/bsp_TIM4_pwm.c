...
//	DMA_InitTypeDef DMA_InitStructure;
	...
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource6,GPIO_AF_TIM4);   //用引脚PC6；把TIM4引到AF2
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource7,GPIO_AF_TIM4);   //用引脚PC7；把TIM4引到AF2    
...
//	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)&pulse;	            //存储器基地址为SRC_Buffer变量的位置
...
//	DMA_Init(DMA1_Stream6, &DMA_InitStructure);
    ...
    //时基配置：
        ...
        //注：这里注释的计算过程是错的（主要因为老是要改），自己对着实际需要的时基自己再算吧
	TIM_TimeBaseInitStruct.TIM_Prescaler=7*24-1;                               //时基结构体的预分频数24Mhz(168M/7=24M)
    ...
    TIM_OC1Init(TIM4,&TIM_OCInitStruct), TIM_OC2Init(TIM4,&TIM_OCInitStruct);
...
//        pulse=LIMIT_MAX_MIN(pulse,1000,0);
//        
