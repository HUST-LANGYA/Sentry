...
{
     delay_ms(600);              //随便延时一下
     FrictionWheel_Set(400,400);
     delay_ms(1000);        
     FrictionWheel_Set(600,600);           
...
//改变占空比，设置摩擦轮转速
void FrictionWheel_Set(uint16_t tmpAccelerator0, uint16_t tmpAccelerator1)
{
    //限幅
    ...
    //这里放成变量是方便调试时观察
    //赋值
    TIM_SetCompare1(TIM4,accelerator0);
    TIM_SetCompare2(TIM4,accelerator1);
}
