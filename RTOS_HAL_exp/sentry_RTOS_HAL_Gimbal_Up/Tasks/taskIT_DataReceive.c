...
extern CanRxMsg CAN1_Rx0_Header, CAN1_Rx1_Header, CAN2_Rx0_Header, CAN2_Rx1_Header;
...
/**
  ...
  * @retval None
  */
void CAN1_Rx1_DataDecode(void) //接收底盘数据
{
    if (CAN1_Rx1_Header.StdId == SHOOTING_HEAT_ID)
    {
        /*在这里接收枪口热量的数据*/
    }
}
...
/**
  ...
  */
//int32_t rx_test = 0;
void CAN2_Rx0_DataDecode(void)
{
    extern gimbal_motor_t MotoPitch;
    extern gimbal_motor_t MotoYaw;
    extern _2006_motor_t BodanMotor;
    extern block_disconnect_t block_disconnect;
    if (CAN2_Rx0_Header.StdId == MotoPitch_ID)
    {
        //block_disconnect.Pitch_Last_Cnt = GetSysCnt();
        MotoPitch.Angle_ABS = CAN2_Rx0_Msg.Data[0] << 8 | CAN2_Rx0_Msg.Data[1];
        MotoPitch.real_speed = CAN2_Rx0_Msg.Data[2] << 8 | CAN2_Rx0_Msg.Data[3];
        MotoPitch.real_flow = CAN2_Rx0_Msg.Data[4] << 8 | CAN2_Rx0_Msg.Data[5];
        //rx_test++;
    }
    else if (CAN2_Rx0_Header.StdId == MotoYaw_ID)
    {
        //block_disconnect.Yaw_Last_Cnt = GetSysCnt();
        MotoYaw.Angle_ABS = CAN2_Rx0_Msg.Data[0] << 8 | CAN2_Rx0_Msg.Data[1];
        MotoYaw.real_speed = CAN2_Rx0_Msg.Data[2] << 8 | CAN2_Rx0_Msg.Data[3];
        MotoYaw.real_flow = CAN2_Rx0_Msg.Data[4] << 8 | CAN2_Rx0_Msg.Data[5];
    }
    else if (CAN2_Rx0_Header.StdId == BodanMotor_Up_ID)
    {
       //block_disconnect.Bodan_Last_Cnt = GetSysCnt();
       BodanMotor.Angle_ABS = CAN2_Rx0_Msg.Data[0] << 8 | CAN2_Rx0_Msg.Data[1];
       BodanMotor.RealSpeed = CAN2_Rx0_Msg.Data[2] << 8 | CAN2_Rx0_Msg.Data[3];
    }
...
float gz10e6;  //?????
void CAN2_Rx1_DataDecode(void) //接收陀螺仪
{
    if (CAN2_Rx1_Header.StdId == Gyro_Pitch_ID)
    {
        memcpy(&Gyro_White.PITCH, &CAN2_Rx1_Msg.Data[0], 4);
        memcpy(&Gyro_White.GY, &CAN2_Rx1_Msg.Data[4], 4);
        gyro_cnt++;
        gyro_pitch_test = Gyro_White.GY * 100000;
        //cnt1 = xTaskGetTickCountFromISR() - cnt_temp;
        //cnt_temp = syscnt;
    }
    if (CAN2_Rx1_Header.StdId == Gyro_Yaw_ID)
    {
        memcpy(&Gyro_White.YAW_ABS, &CAN2_Rx1_Msg.Data[0], 4);
        memcpy(&Gyro_White.GZ, &CAN2_Rx1_Msg.Data[4], 4);
